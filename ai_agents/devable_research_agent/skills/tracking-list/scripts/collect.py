#!/usr/bin/env python3
"""devable-research-agent data collection orchestrator.
...
def collect_tavily(config: Dict[str, Any], from_date: str, to_date: str, depth: str) -> CollectionResult:
    """Collect X/Twitter and web signals via Tavily Search API."""
    from lib import tavily_collector

    api_key = env.get_key(config, "TAVILY_API_KEY")
    return tavily_collector.collect(
        entities.X_HANDLES_OFFICIAL,
        entities.X_HANDLES_KEY_PEOPLE,
        entities.WEB_QUERIES,
        from_date,
        to_date,
        api_key,
        depth,
        x_handles_kol=entities.X_HANDLES_KOL,
    )


def collect_goodailist(config: Dict[str, Any], from_date: str, to_date: str, depth: str) -> CollectionResult:
    """Scrape trending AI repos from Good AI List."""
    from lib import goodailist_scraper
    return goodailist_scraper.collect(
...
def collect_ainews(config: Dict[str, Any], from_date: str, to_date: str, depth: str) -> CollectionResult:
    """Collect AI engineering signals from AINews (smol.ai) RSS."""
    from lib import ainews_collector
...
def run_collection(
    ...
    Args:
        ...
        skip: Optional list of sources to skip (default: none — all collectors run).
            Deny-list semantics: forcing the caller to name what they skip
    ...
    if unknown:
        _log(f"Warning: unknown collector(s) in --skip (ignored): {sorted(unknown)}")
    known_skip = skip_set & COLLECTORS.keys()
    active_collectors = {
        name: func for name, func in COLLECTORS.items() if name not in known_skip
    }
    if known_skip:
        _log(f"Skipping collectors: {sorted(known_skip)}")
...
def main():
    parser = argparse.ArgumentParser(description="devable-research-agent data collection")
    ...
    parser.add_argument("--skip", nargs="*", default=[],
                        help="Sources to SKIP (default: none — all collectors run). "
